...
\usepackage{caption}
\captionsetup{
  font=footnotesize,    % same size as IEEE’s default
  labelfont=normal,     % not scshape
  textfont=normal       % not scshape
}
...
\Comments=0 % TO$DO$: set to 0 for final version
\usepackage{color}
...
\newcommand{\tillman}[1]{\kibitz{violet} {[Tillman: #1]}}
\newcommand{\vitaly}[1]{\kibitz{orange} {[vitaly: #1]}}
...
Kizito Salako$^*$, 
Maksim Shekhunov$^\dag$, 
Vitaly Aksenov$^*$, 
Tillman Weyde$^*$}

\IEEEauthorblockA{*City St George's University of London, UK, \dag ITMO University, St Petersburg, Russia 
%\\\texttt{\{enrico.lopedoto, k.o.salako, vitaly.aksenov, t.e.weyde\}@city.ac.uk,shexunov@gmail.com}
} 
...
\enrico{REVIEW TO ADDRESS
\begin{itemize}
 %\item 1. The paper lacks a clear theoretical explanation of why aligning model derivatives with data derivatives improves generalization.\enrico{no time and vague}
 %\item 2. The finite-difference method used to estimate derivatives requires extra forward passes, increasing computational cost. A detailed complexity analysis would help clarify the trade- offs.\enrico{maybe we can expand with a few sentences regading the complexity}
 %\item 3. The scalability of DLoss to large datasets and deep neural networks is not discussed. Given the extra computation involved, its practicality for high-dimensional problems is unclear. \enrico{we can discuss briefly}
 %\item 4. The paper does not compare DLoss with modern adaptive regularization techniques like Batch Normalization, Adaptive Gradient Clipping, or Jacobian regularization. \enrico{we can discuss briefly on batch and clipping}
 \item 5. The impact of hyperparameter choices on DLoss is not explored, and an ablation study would help understand its sensitivity. \enrico{we said perhaps add a small ablation on some hyperparameter with 1 graph. lets discuss which graph to create}
 %\item Some mathematical derivations could be explained more clearly, focusing on the Derivative Error section.\enrico{no time and vague}
 %\item The experiment section could be more concise to highlight the key findings.\enrico{no time and vague}
 \item You could also discuss the computational trade-offs of using this system rather than other traditional methods\enrico{as before}
\end{itemize}
}
...
Another widely used regularization method (but only for neural networks) is \emph{DropOut}, where there is no additional loss term but neurons are randomly switched off during training~\cite{hinton2012L2,srivastava2014dropout}.
...
\begin{enumerate}
 \item We propose \emph{DLoss}, a new regularization method for regression models;
 \item We propose two variants of \emph{DLoss} using nearest neighbor or random selection of data points;
 \item We provide a publicly available implementation in PyTorch, available at \url{https://github.com/EnricoLope/DLoss};
 \item We evaluate our approach on different datasets and compare it to training using no regularization, $L_2$ regularization, and Dropout.
\end{enumerate}
...
Since we are using the derivatives in our approach we also address the related work on Neural Differential Equations.
...
While we believe that this our approach is novel, there are various types of models in the literature that integrate differential equations with neural networks in other ways. 
...
We want to minimize the difference between the model derivative and the estimated target derivative; i.e., we want to minimize the value of a \emph{DLoss} function, which we will define below.
...
Using training data consisting of input-output pairs $(\mathbf{x}_i,y_i)$ for $i=1,\ldots,n$, where $n$ is the number of training data points, regression modeling aims to optimize the parameter vector $\boldsymbol{\beta}$ of this function $f$, so that for general input-output pairs $(\mathbf{x},y)$, 
\begin{equation}
...
In this paper, we refer to an input vector $\mathbf{x}$ as a point, to $(\mathbf{x},y)$ as a pair, and to two pairs $((\mathbf{x}_i,y_i),(\mathbf{x}_j,y_j))$ as a tuple.
...
We propose a new regularization term, the \emph{DLoss}, that is defined on the difference between the model's derivative (analytically or numerically calculated) and the target's derivative (estimated from the training data).
...
\end{equation} 
where $\tilde{\mathbf{v}} = \frac{\mathbf{v}}{\|\mathbf{v}\|_2}$ is normalized to unit length and $\varepsilon$ is a small constant scalar. 
...
These tuples are used to estimate derivatives in section \ref{subsec_optimization}. 
...
We determine the $l$ nearest neighbors using the efficient KD-tree algorithm~\cite{bentley1975multidimensional,nnalgo}. 
%. Once a pair is selected as a nearest neighbor, the next nearest neighbor is selected from the remaining pairs in the training set, until $l$ neighbors in total have been selected. 
...
\begin{table}[tb]
 ...
 \end{small}
 \end{center}
 \caption{Datasets used in our experiments. All datasets have a single real-valued output per row. 
 The number of input values is listed in column \textsc{Inputs} and \textsc{Rows} is the number of input-output pairs in the dataset. 
 \textsc{Library} describes the Python package from which the dataset is obtained: \emph{STM} is for statsmodels (\url{https://www.statsmodels.org/}), \emph{Scikit} for scikit-learn (\url{https://scikit-learn.org/stable/}) and \emph{OML} for OpenML (\url{https://www.openml.org/}).
 The source for ``modechoice'' dataset is \cite{greene2011modechoice}, ``anes96'' dataset \cite{anes1996}, ``diabetes'' dataset \cite{efron2004least}, ``cancer'' dataset \cite{rice_cancer} and ``wine'' dataset \cite{cortez2009modeling}.
 ...
 In the generation of the datasets, no noise has been added.
 Further details of the data generating functions can be found in the library documentation available at \url{https://scikit-learn.org/stable/datasets/}.
 }
 \label{tab:data-table} 
\end{table}

For noiseless data, we created synthetic datasets using data generation functions available from the Scikit-Learn library, where we have selected F1, Regression1, Regression10, Sparse-uncorrelated, and Swiss Roll. 
...
\autoref{tabresults} shows the results of our $DL_{RND}$ and $DL_{NN}$ methods as well as $STD$, $L_2$ and $DO$ training. 
$DL_{NN}$ achieves the best generalization errors $MSE_{val}$ for 5 of the 10 datasets.
...
However, $DL_{NN}$ errors are significantly lower than $DL_{RND}$, $DO$ and $STD$, and all regularized models are significantly better than $STD$, as expected.
...
\begin{table}[tb]
 ...
 & & \multicolumn{1}{c}{$L_2$} & \multicolumn{1}{c}{$DL_{RND}$} & \multicolumn{1}{c}{$DO$} & \multicolumn{1}{c}{$STD$} \\ 
 \midrule
 $DL_{NN}$ & 0.716 & - & * & * & ** \\
 $L_2$ & 0.570 & & - & - & ** \\
 $DL_{RND}$ & 0.408 & & & - & ** \\
 ...
 \end{small}
 \end{center} 
 \caption{The Mean Reciprocal Rank (MRR) values of the different models and their mutual comparisons (higher is better). 
 `*' or `**' indicate a significant difference, according to a Wilcoxon signed rank test over the model ranks per dataset as shown in \autoref{tabrank}, at the 5\% or 1\% level, respectively.}
 \label{tabranktest}
...
In our experiments the $DL_{NN}$ method showed the best results as measured by average and mean reciprocal rank (MRR). 
...
Correspondingly, the computation time for \emph{DLoss} is generally higher than the $STD$ models (see $t$ values in \autoref{tabresults}).
However, the computation times show that all regularization methods require additional time on average, and that the amount is quite variable.
It may be possible to reduce the computational cost by not determining the model derivative at the midpoint but reusing data points instead or by using analytical gradient calculations, which would be model dependent. 
...
\begin{table}[tb]
 ...
 \caption{The results of significance tests for differences of $MSE_{val}$ between $STD$ and \emph{DLoss} methods. 
 ...
 For $STD$ and $DL$ training we have 2 groups, therefore we run all combinations, giving us 6 results for each, real and synthetic data.
 ...
 The low $p$-values of the Wilcoxon test indicate that the $STD$ groups' $MSE_{val}$ are significantly greater than the $DL$ groups' (indicated with an asterisk *). 
 The $^{*},^{**},^{***}$ indicate the significance at 5\%, 1\% and 0.1\% 
 levels, respectively.
 We also applied a paired t-test. 
 However, the t-test is only strictly valid for normally distributed data, and the Shapiro-Wilk test indicates that the distribution is significantly different from a normal distributions in most cases here. 
 }
 \label{tabtests}
\end{table}
...
We have proposed a new regularization method, \emph{DLoss}, which aims to reduce the difference between model derivatives and target function derivatives by using differential information inferred from training data.
...
The metric we use is the MSE on validation sets to compare the performance of different regularization methods. 
We observe an overall generalization improvement using \emph{DLoss} with nearest neighbor selection ($DL_{NN}$). 
With this method, the generalization as measured by mean reciprocal rank is better than the established $L_2$ and Dropout regularization. 

The improved generalization requires additional computation, roughly, but the results in practice are quite variable between the different regularization methods and algorithmic optimizations may help to reduce the cost. 
Future work on this approach will include tests with more models and more datasets and exploration of combined regularization methods as well as the formulation of \emph{DLoss} for classification problems. 
Higher dimensional datasets with more data points and more model architectures will be explored in order to determine how far the benefits of using the \emph{DLoss} method generalize in these situations. 
